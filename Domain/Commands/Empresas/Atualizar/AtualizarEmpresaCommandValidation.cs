...
namespace Domain.Commands.Empresas.Atualizar
{
    public class AtualizarEmpresaCommandValidation : AbstractValidator<AtualizarEmpresaCommand>
    {
        ...
        public AtualizarEmpresaCommandValidation(IUnitOfWork unitOfWork)
        {
            _unitOfWork = unitOfWork;

            RuleFor(c => c)
               .SetValidator(new EmpresaCommandValidation<AtualizarEmpresaCommand>(_unitOfWork));
        ...
        {
            return id.HasValue && await _unitOfWork.EmpresaRepository.ExisteIdAsync(id.Value);
        }
    }
}
