...
namespace Domain.Commands.Produtos.Atualizar
{
    public class AtualizarProdutoCommandValidation : AbstractValidator<AtualizarProdutoCommand>
    {
        ...
        public AtualizarProdutoCommandValidation(IUnitOfWork unitOfWork)
        {
            _unitOfWork = unitOfWork;

            RuleFor(c => c)
               .SetValidator(new ProdutoCommandValidation<AtualizarProdutoCommand>(_unitOfWork));
        ...
        {
            return id.HasValue && await _unitOfWork.ProdutoRepository.ExisteIdAsync(id.Value);
        }
    }
}
